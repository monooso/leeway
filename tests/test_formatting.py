...
from app.formatting import format_reset_time, truncate_error
...
class TestFormatResetTime:
    ...
    def test_none_returns_dash(self):
        assert format_reset_time(None, now=NOW) == "—"

    def test_past_returns_now(self):
        past = NOW - timedelta(minutes=5)
        assert format_reset_time(past, now=NOW) == "now"

    def test_seconds_away_returns_less_than_1m(self):
        soon = NOW + timedelta(seconds=30)
        assert format_reset_time(soon, now=NOW) == "< 1m"

    def test_minutes_away(self):
        assert format_reset_time(_future(42 * 60), now=NOW) == "42m"

    def test_hours_and_minutes(self):
        assert format_reset_time(_future(3 * 3600 + 15 * 60), now=NOW) == "3h 15m"

    def test_days_and_hours(self):
        assert format_reset_time(_future(2 * 86400 + 5 * 3600), now=NOW) == "2d 5h 0m"

    def test_exactly_one_minute(self):
        assert format_reset_time(_future(60), now=NOW) == "1m"

    def test_exactly_24_hours_shows_days(self):
        assert format_reset_time(_future(24 * 3600), now=NOW) == "1d 0h 0m"

    def test_24_hours_30_minutes(self):
        assert format_reset_time(_future(24 * 3600 + 30 * 60), now=NOW) == "1d 0h 30m"

    def test_over_24_hours(self):
        assert format_reset_time(_future(25 * 3600), now=NOW) == "1d 1h 0m"

    def test_days_hours_and_minutes(self):
        assert format_reset_time(_future(2 * 86400 + 5 * 3600 + 17 * 60), now=NOW) == "2d 5h 17m"
    ...
    def test_defaults_to_real_time_when_now_omitted(self):
        ...
        result = format_reset_time(far_future)
...
class TestTruncateError:
    ...
    def test_short_message_unchanged(self):
        assert truncate_error("Something went wrong") == "Something went wrong"

    def test_long_message_truncated(self):
        long = "x" * 200
        result = truncate_error(long, max_length=100)
        assert len(result) == 100
        assert result.endswith("...")

    def test_message_at_limit_unchanged(self):
        msg = "x" * 100
        assert truncate_error(msg, max_length=100) == msg
